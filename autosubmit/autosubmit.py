...
class Autosubmit:
    ...
    def expid(
        ...
        try:
            ...
            Log.warning(f"Could not update the status of experiment {exp_id}. "
    ...
    def run_experiment(expid: str, start_time: str | None = None, start_after: str | None = None,
        ...
        try:
            ...
            try:
                ...
                Log.warning(f"Autosubmit couldn't update the final experiment status for {expid}: {str(e)}")
        ...
        try:
            ...
            if template_file_path is not None:
                if not os.path.exists(template_file_path):
                    raise AutosubmitCritical(
                        f"Template {template_file_path} does not exist ", 7014)
    ...
    def archive(expid, noclean=True, uncompress=True, rocrate=False):
        ...
        try:
            ...
            Log.warning(f"Could not mark experiment {expid} as archived. "
                        f"The status will be fixed by the API worker. Error: {str(e)}")
        return True
    ...
    def unarchive(experiment_id, uncompressed=True, rocrate=False):
        ...
        try:
            ...
            Log.warning(f"Could not update the status of experiment {experiment_id}. "
                        f"The status will be fixed by the API worker. Error: {str(e)}")
            ...
            with Lock(os.path.join(tmp_path, 'autosubmit.lock'), timeout=1) as fh:
                try:
                    ...
                    if not os.path.exists(os.path.join(exp_path, "db")):
                        raise AutosubmitCritical(f"The db folder does not exist. Make sure that the 'db'"
                                                 f" folder exists in the following path: {exp_path}", code=6013)
                    if not os.path.exists(os.path.join(exp_path, "plot")):
                        raise AutosubmitCritical(f"The plot folder does not exist. Make sure that the 'plot'"
                                                 f" folder exists in the following path: {exp_path}", code=6013)
                    ...
                    try:
                        ...
                        Log.warning(f"Could not update the status of experiment {expid}. "
                                    f"The status will be fixed by the API worker. Error: {str(e)}")
