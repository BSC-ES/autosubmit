...
def _check_uncommitted_code(git_repo: Path) -> bool:
    ...
    :return: ``True`` if there are no issues with the repository (i.e. code committed), ``False`` otherwise.
    """
...
def _check_code_not_pushed(git_repo: Path) -> bool:
    """Check if there is any code not pushed to remotes in the given Git repository or submodules.

    :param git_repo: Path to the Git repository.
    :return: ``True`` if there are no issues with the repository (i.e. code pushed), ``False`` otherwise.
    """
    encoding = locale.getlocale()[1]
...
def check_unpushed_changes(expid: str, as_conf: AutosubmitConfig) -> None:
    ...
    if expid[0] == 'o' and project_type == 'git':
        ...
        if not _check_uncommitted_code(proj_dir):
            raise AutosubmitCritical("You must commit and push your code to the remote Git repository in an "
                                     "operational experiment before running it", 7075)

        if not _check_code_not_pushed(proj_dir):
