...
Autosubmit runs a performance check on PRs labeled `perf-benchmark`: the PR is
...
* Maintainers can comment `/metrics_full` for the heavy suite, and
  ...
  `.benchmarks/allowed-users.txt` on the default branch, only). `/metrics_promote` requires
