...
def test_job_package_timeout(platform_class , expected_timeout, create_platform):
    ...
    We use a fixed list of four jobs, with wallclocks of 5 seconds.
    The ``platform_class`` is used to decide the type of the platform
    to be set in each job.

    As every job has the same timeout, the timeout (for local and ps) will
    be calculated based on 5. Autosubmit adds 30% buffer to the wallclock
    ...
    platform.EXECUTION_MODE = platform_class.EXECUTION_MODE
    for job in jobs:
...
def test_job_package_execution_mode_for_slurm_with_serial_platform(autosubmit_config, tmp_path):
    """Test the execution mode is correct for a Slurm platform with a serial platform.

    We have a Slurm platform used in jobs. The Slurm platform has a serial platform
    of type PS.

    The slurm platform execution mode is BATCH. The PS platform execution mode is DIRECT.

    We want to make sure that the job package created uses the Slurm platform given,
    and not the PS platform.
    """
    as_conf = autosubmit_config(
        ...
        experiment_data={
            ...
            "JOBS": {
                "job1": {"script": "echo OK", "PLATFORM": "SLURM", "PROCESSORS": 1},
                "job2": {"script": "echo OK", "PLATFORM": "SLURM", "PROCESSORS": 1},
                "job3": {"script": "echo OK", "PLATFORM": "SLURM", "PROCESSORS": 1},
                "job4": {"script": "echo OK", "PLATFORM": "SLURM", "PROCESSORS": 1},
            },
        },
    )

    paramiko_submitter = ParamikoSubmitter(as_conf)

    slurm_platform = paramiko_submitter.platforms['SLURM']
