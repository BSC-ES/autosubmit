...
def _format_command(argv: Sequence[str]) -> str:
    ...
    :param argv: The command-line arguments, including the executable.
    :return: The formatted command line.
    """
    return shlex.join(_redact_arguments(argv))


def log_command_context(argv: Sequence[str]) -> None:
    ...
    Log.info(f"  Command: {_format_command(argv)}")
