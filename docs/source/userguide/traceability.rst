...
In order to facilitate the next steps the complete list of `PARAMETERS` and it's description for git
...
.. list-table:: Parameters Description
   ...
   * - **GIT.PROJECT_SUBMODULES**
     - Creates a folder within the main project that allows for a ``subproject`` being located within the ``superproject`` keeping the history and content split and only giving availability to the a specific commit
   * - **GIT.PROJECT_SUBMODULES_DEPTH**
     - Allow for a shallow clone to be done within the subproject with the specified depth. This will allow for smaller local clones, but limits its commit reachability
