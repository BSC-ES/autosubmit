...
def test_ini_to_yaml_backup_exists(tmp_path: "LocalPath", mocker):
    ...
    assert backup_file_size == backup_file.stat().st_size, (
        "ini_to_yaml replaced the existing backup file"
    )
...
def test_upgrade_scripts_as_3_conf_causes_error(
    autosubmit_exp, tmp_path: "LocalPath", as3_ini_files: list[Path], mocker
):
    ...
    mocker.patch(
        "autosubmit.config.upgrade_scripts.ini_to_yaml", side_effect=ValueError("test")
    )
    mocked_log = mocker.patch("autosubmit.config.upgrade_scripts.Log")
...
@pytest.mark.parametrize(
    "fail_on_template,expected_error",
    [
        (False, "Failed to fix placeholders in the new AS4 YAML file"),
        (True, "Failed to fix placeholders in template file"),
    ],
    ids=["First YAML file fails", "First template file fails"],
)
def test_upgrade_scripts_as_3_yaml_placeholders_cause_error(
    fail_on_template: bool,
    expected_error: str,
    autosubmit_exp,
    tmp_path: "LocalPath",
    ...
    * We mock, so the first template (index 3) raises a value error, with the message that the template failed
    ...
    # There are 3 YAML files followed by 1 template.
    side_effect: list[Any] = [([], []) for _ in as3_ini_files]

    if fail_on_template:
        side_effect.append(ValueError("test"))
    else:
        side_effect[0] = ValueError("test")
        side_effect.append(([], []))

    mocked_fix = mocker.patch(
        "autosubmit.config.upgrade_scripts._fix_placeholders",
    ...
    assert mocked_fix.call_count == 4
    assert mocked_log.warning.called
    assert any(
        expected_error in call.args[0] for call in mocked_log.warning.call_args_list
    )


def test_upgrade_scripts(
    autosubmit_exp, tmp_path: "LocalPath", as3_ini_files: list[Path], mocker
):
    ...
    as_exp = autosubmit_exp(
        experiment_data={
            ...
            "LOCAL": {"PROJECT_PATH": str(temp_project)},
        },
        create=False,
    )
    ...
    as3_script_template.write_text(
        dedent("""\
    #!/bin/bash
    ...
    assert create(as_exp.expid, force=True, noplot=True, hide=True) == 0
    ...
    assert create(as_exp.expid, force=True, noplot=True, hide=True) == 0
    assert inspect(
        as_exp.expid, lst="", filter_chunks="", filter_status="", filter_section=""
    )

    inspect_generated_script = exp_dir / f"tmp/{as_exp.expid}_LOCAL_SETUP.cmd"
    assert inspect_generated_script.exists(), (
        f"inspect executed but {inspect_generated_script} does not exist"
    )

    assert (
        f"The job name is {as_exp.expid}_LOCAL_SETUP"
        in inspect_generated_script.read_text()
    )

    as_conf.reload(force_load=True)

    # The %user% variable will be found in multiple platforms that contain variables ending
    # with ``SOMETHING.SOMETHING.USER``. There will be a warning to the user telling them to
    # select one of the variables, and the raw/original value is kept. Thus, lower case user.
    assert as_conf.experiment_data["PLATFORMS"]["MARENOSTRUM4-TEST"]["USER"] == "%user%"
    assert mocked_log.error.called
    assert any(
        "Duplicate variable found: USER" in str(c)
        for c in mocked_log.error.call_args_list
    )
    ...
    with open(upgraded_local_script) as f:
        new_local_script_content = f.read()
    assert "%ROOTDIR%" in new_local_script_content, (
        "The %rootdir% variable was not upgraded/upper-cased!"
    )
