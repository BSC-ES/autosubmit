...
class ParamikoPlatform(Platform):
    ...
    def __init__(self, expid: str, name: str, config: dict, auth_password: Optional[Union[str, list[str]]] = None):
        ...
        self.remove_files_on_transfer = False
        if self.config:
            self.remove_files_on_transfer = self.config.get(
                self.config.get("PLATFORMS", {})
                .get(self.name.upper(), {})
                .get("REMOVE_FILES_ON_TRANSFER", False)
            )
    ...
    def get_file(self, filename, must_exist=True, relative_path='', ignore_log=False, wrapper_failed=False):
        ...
        try:
            ...
            if self.remove_files_on_transfer and self._checksum_validation(
