...
class CommandDocstring:
    ...
    def short_description(self) -> str:
        ...
        return self.description.splitlines()[0] if self.description else ""
...
def _prioritise_positional_argument(
    parser: ArgumentParser,
    name: str,
) -> None:
    """Make a positional argument the first positional argument."""
    positionals = [action for action in parser._actions if not action.option_strings]

    if not any(action.dest == name for action in positionals):
        return

    positionals.sort(key=lambda action: action.dest != name)

    # Reorder positional actions in the parser.
    positional_iter = iter(positionals)
    for index, action in enumerate(parser._actions):
        if not action.option_strings:
            parser._actions[index] = next(positional_iter)

    # Reorder positional actions in the positional argument group.
    parser._positionals._group_actions.sort(key=lambda action: action.dest != name)
...
def add_common_arguments(
    ...
    *,
    ...
    if _has_argument(parser, "expid"):
        # Make expid the first positional argument.
        _prioritise_positional_argument(parser, "expid")
...
def parse_docstring(docstring: str | None) -> CommandDocstring:
    """Parse a command docstring.

    Returns empty values if no docstring is provided.

    :param docstring: The docstring to parse.
    :return: The parsed docstring.
    """
    if not docstring:
        return CommandDocstring(title="", description="")
...
class CommandGroup(str, Enum):
    ...
    CONFIGURATION = "Configuration and installation"
    GENERAL = "General"


def cli_unknown_command(
    command: str,
    commands: dict,
) -> None:
    """Print an error message for an unknown command.

    If a command with a similar name exists, a suggestion is included.

    :param command: The command name entered by the user.
    :param commands: Mapping of available command names to entry points.
    """
    ...
    Log.error(f"Error: unknown autosubmit sub-command '{command}'.")

    if matches:
        Log.error(f"Did you mean 'autosubmit {matches[0]}'?")
