...
def test_copying_experiment_with_hpc_in_command(autosubmit_exp: Callable, tmp_path):
    ...
    assert create(exp_id, noplot=True, hide=True) == 0
...
def test_copy_expid_with_main_yaml(tmp_path: Path, autosubmit_exp):
    ...
    assert create(exp.expid, noplot=True, hide=True) == 0
...
def test_create_expid_default_hpc():
    ...
    describe_output: tuple[str, str | datetime, str, str, str] = describe(experiment_id)  # type: ignore
    hpc_result = describe_output[4].lower()
...
def test_create_expid_flag_hpc(fake_hpc: str, expected_hpc: str):
    ...
    # capture the platform using the "describe"
    describe_experiment: str = cast(str, cast(object, describe(experiment_id)))
...
def test_copy_expid_with_flag_hpc(
    ...
    describe_output: tuple[str, str | datetime, str, str, str] = describe(exp_id)  # type: ignore
    hpc_experiment = describe_output[4].lower()

    describe_copy: tuple[str, str | datetime, str, str, str] = describe(copy_exp)  # type: ignore
...
def test_copy_expid_fn(fake_hpc: str, expected_hpc: str):
    ...
    # capture the platform using the "describe"
    describe_copy: tuple[str, str | datetime, str, str, str] = describe(copy_id)  # type: ignore
...
def test_copy_expid_no():
    ...
    # capture the platform using the "describe"
    describe_output: tuple[str, str | datetime, str, str, str] = describe(  # type: ignore
        copy_experiment_id
    )
    hpc_result = describe_output[4].lower()
...
def test_autosubmit_generate_config(
    mocker, tmp_path, get_next_expid: Callable[[], str]
):
...
def test_autosubmit_generate_config_resource_listdir_order(mocker):
