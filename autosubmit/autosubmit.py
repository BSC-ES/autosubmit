    ...
    def _copy_code(as_conf: AutosubmitConfig, expid: str, project_type: str, force: bool) -> bool:
        ...
        if project_type == "git":
            ...
            if os.path.exists(local_proj_dir):
                Log.info("Using project folder: {0}", local_proj_dir)
                if not force:
            ...
            Log.debug("The project folder {0} has been created.", local_proj_dir)
            Log.info("Checking out revision {0} into {1}",
                     svn_project_revision + " " + svn_project_url, local_proj_dir)
            try:
