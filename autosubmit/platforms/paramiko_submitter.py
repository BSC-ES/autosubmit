...
class ParamikoSubmitter:
    ...
    def load_local_platform(self, as_conf: AutosubmitConfig, exp_data: Optional[dict] = None,
        ...
        Adds the platform to the dictionary of platforms, ``.platforms``.
        """
        if exp_data is None:
            exp_data = BasicConfig().props()
        local_platform = LocalPlatform(as_conf.expid, 'local', exp_data, auth_password=auth_password)
    ...
    def load_platforms(self, as_conf: 'AutosubmitConfig',
                       auth_password: Optional[Union[str, list[str]]] = None) -> None:
        """Create all the platforms object that will be used by the experiment.

        :param as_conf: Autosubmit configuration.
        :param auth_password: 2FA password, passed to the Slurm platform only.
        """
        ...
        self.load_local_platform(as_conf=as_conf, exp_data=exp_data)
        ...
        for section in platform_data:
            ...
            try:
                if platform_type == 'ps':
                    remote_platform = PsPlatform(as_conf.expid, section, exp_data)
                elif platform_type == 'ecaccess':
                    remote_platform = EcPlatform(as_conf.expid, section, exp_data, platform_version)
                elif platform_type == 'slurm':
                    remote_platform = SlurmPlatform(as_conf.expid, section, exp_data, auth_password=auth_password)
                elif platform_type == 'pjm':
                    remote_platform = PJMPlatform(as_conf.expid, section, exp_data)
                else:
            ...
            remote_platform.max_waiting_jobs = platform_data[section].get(
                'MAX_WAITING_JOBS', platform_data[section].get('MAXWAITINGJOBS', as_conf.get_max_waiting_jobs()))
            remote_platform.total_jobs = platform_data[section].get(
                'TOTAL_JOBS', platform_data[section].get('TOTALJOBS', as_conf.get_total_jobs()))
