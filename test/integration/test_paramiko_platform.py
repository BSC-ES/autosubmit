...
@pytest.mark.docker
def test_send_command_timeout_error_exec_command(cmd: str, timeout: Optional[int], ps_platform: PsPlatform, mocker, tmp_path, make_ssh_client):
...
@pytest.mark.docker
def test_exec_command_after_a_reset(ps_platform: PsPlatform, mocker, make_ssh_client):
...
@pytest.mark.docker
def test_exec_command_ssh_session_not_active(x11, retries, ps_platform: PsPlatform, mocker, make_ssh_client):
...
@pytest.mark.docker
def test_exec_command_socket_error(ps_platform: PsPlatform, mocker, make_ssh_client):
...
@pytest.mark.docker
def test_exec_command_ssh_session_not_active_cannot_restore(ps_platform: PsPlatform, mocker, make_ssh_client):
...
@pytest.mark.docker
def test_fs_operations(ps_platform: PsPlatform, mocker, make_ssh_client, tmp_path):
...
@pytest.mark.docker
def test_test_connection(ps_platform: PsPlatform, mocker, make_ssh_client, tmp_path):
    """Test that we can access files, send new files, move, delete."""
    user = getuser() or "unknown"
    ...
    with DockerContainer(image=_DOCKER_IMAGE, remove=True, hostname='openssh-server') \
        ...
        assert None == ps_platform.test_connection(None)
