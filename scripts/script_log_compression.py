...
for i, f in enumerate(filenames):
    for _ in range(t):
        ...
        time_zstd[i] += elapsed
        if elapsed > max_zstd[i]: max_zstd[i] = elapsed
        if elapsed < min_zstd[i]: min_zstd[i] = elapsed
        ...
        elapsed = end_time-start_time
        time_xz[i] += elapsed 
        if elapsed > max_xz[i]: max_xz[i] = elapsed
        if elapsed < min_xz[i]: min_xz[i] = elapsed
        ...
        if elapsed > max_zipfile[i]: max_zipfile[i] = elapsed
        if elapsed < min_zipfile[i]: min_zipfile[i] = elapsed
