...
from autosubmit.scripts.autosubmit import _autosubmit, _get_arg_parser
...
def test_no_command_shows_help(mocker):
    ...
    mock_help = mocker.patch("autosubmit.scripts.autosubmit._cli_help")
...
def test_invalid_subcommand(mocker):
    """Test that an unknown subcommand is rejected with a helpful error."""
    mock_help = mocker.patch("autosubmit.scripts.autosubmit._cli_help")
    mock_unknown_command = mocker.patch(
        "autosubmit.scripts.autosubmit.cli_unknown_command"
    )

    status = _autosubmit(["versioning"])

    assert status == 2
    mock_unknown_command.assert_called_once()

    args = mock_unknown_command.call_args.args
    assert args[0] == "versioning"
...
def test_top_level_version(mocker):
    ...
    mock_version = mocker.patch("autosubmit.scripts.autosubmit._cli_version")
...
def test_unknown_subcommand_does_not_trigger_argparse_error(mocker):
    """Test that an unknown command reaches Autosubmit's command handling."""
    commands = {
        "archive": mocker.Mock(),
        "create": mocker.Mock(),
    }
    ...
    status = _autosubmit(["archivo"])

    assert status == 2
    mock_unknown_command.assert_called_once_with(
        "archivo",
        commands,
    )
