...
from autosubmit.database import database_backup, db_common
...
def _delete_expid(expid_delete: str, force: bool = False) -> None:
    ...
    if not (is_owner or (force and is_eadmin)):
        if not is_eadmin:
            raise AutosubmitCritical(
...
def report(
    ...
    try:
        ...
        if show_all_parameters:
            ...
            parameter_file = open(os.path.join(tmp_path, parameter_output), "w")
            # parameters
            parameters = as_conf.load_parameters()
...
) -> tuple[str | Any, str | datetime | Any, str | Any, str | Any, str | Any] | None:
