...
.. runcmd:: autosubmit describe a000

``autosubmit report <EXPID>`` extracts the experiment variables. It has two
modes that can be used together:

* ``-all`` writes every available parameter to
  ``<EXPID>_parameter_list_<timestamp>.txt``.
* ``-t <template>`` renders a user-supplied template, substituting
  ``%KEY%`` markers with values, into ``<EXPID>_report_<timestamp>``.
