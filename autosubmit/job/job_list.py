...
class JobList(object):
    ...
    # ADD QOL (check also tests passing, re-added [:] as here we could have all the jobs loaded
    ...
    # ADD QOL ( remove unused parameters)
    def generate(
            self,
    ...
    def build_sections_data_to_store(self) -> list[dict[str, Any]]:
        ...
        for section_name, section_data in sections.items():
            ...
            # # ADD QOL ( maybe tests don't pass as now we have splits)
    ...
    def continue_run(self) -> bool:
        ...
        # Updates job_list
        self.recover_logs()
        save_jobs, save_edges = self.update_list(self._as_conf, submitter=self.submitter)
        if save_jobs:
            self.save_jobs()
        if save_edges:
            self.save_edges()
    ...
    def update_list(
        ...
        if not fromSetStatus:
            save_jobs |= self._update_waiting_and_delayed_jobs(as_conf)
            save_jobs |= self._update_held_jobs(as_conf)
    ...
    # ADD QOL (remove unused parameter)
    def save_wrappers(
            self,
    ...
    # ADD QOL (re-added :, check tests are passing)
