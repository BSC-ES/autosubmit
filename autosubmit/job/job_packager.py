...
class JobPackager(object):
    ...
    def check_if_packages_are_ready_to_build(self):
        ...
        if self.hold and len(jobs_ready) > 0:
            ...
            jobs_in_held_status = self._jobs_list.get_held_jobs() + self._jobs_list.get_submitted(self._platform,
                                                                                                  hold=self.hold)
            held_by_id = dict()
    ...
    def build_packages(self):
        ...
        for wrapper_name, jobs in jobs_to_wrap.items():
            ...
            section = self._as_config.experiment_data.get("WRAPPERS", {}).get(self.current_wrapper_section, {}).get(
                "JOBS_IN_WRAPPER", "")
            ...
            if len(packages_to_submit) > 0:
                Log.result(f"Built {len(packages_to_submit)} wrappers for {wrapper_name}")
...
class JobPackagerHorizontal(object):
    ...
    def get_next_packages(self, jobs_sections, max_wallclock=None, potential_dependency=None,
                          packages_remote_dependencies=list(), horizontal_vertical=False, max_procs=0):
        packages = []
        ...
        while self.max_jobs > 0:
            ...
            for job in self.job_list:
                for child in job.children:
                    if job.section == child.section or (
                            job.section in jobs_sections and child.section in jobs_sections.split("&")) \
