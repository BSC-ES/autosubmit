...
class JobsDbManager(DbManager):
    ...
    def clear_unused_nodes(self, differences: Dict[str, Dict[str, Any]]) -> None:
        """
        Clear unused nodes from the database based on configuration differences.

        :param differences: Dictionary with section differences
        :type differences: dict
        """
        ...
        for section_name, section_diff in differences.items():
            ...
            if section_diff.get('status') == 'removed':
                ...
                for job in jobs_dict:
                    ...
                    if 'chunk_number' in section_diff and job['chunk'] is not None:
                        if job['chunk'] > section_diff['chunk_number']:
                            should_delete = True

                    if not should_delete and 'splits' in section_diff and job['split'] is not None:
                        if job['split'] > section_diff['splits']:
                            should_delete = True

                    if not should_delete and 'datelist' in section_diff and job['date'] is not None:
                    ...
                    if not should_delete and 'members' in section_diff and job['member'] is not None:
