...
def _generate_message_cpmip_threshold_violations(
        exp_id: str,
        job_name: str,
        violations: dict[str, dict]) -> str:
    """Generate email body for CPMIP threshold violations.
    
    :param exp_id: The experiment id.
    :type exp_id: str
    :param job_name: The name of the job.
    :type job_name: str
    :param violations: A dictionary containing the CPMIP threshold violations.
                       Each violation dict contains 'threshold', 'accepted_error', 
                       'comparison', 'bound', and 'real_value' keys.
    :type violations: dict[str, dict]
    :return: The email body for CPMIP threshold violations.
    :rtype: str
    """
    
    details = []
    for metric_name in sorted(violations.keys()):
        values = violations[metric_name]
        threshold = values.get("threshold")
        accepted_error = values.get("accepted_error", 0)
        comparison = str(values.get("comparison"))
        bound = values.get("bound")
        real_value = values.get("real_value")

        comparison_label = {
            "greater_than": "must be >= effective bound",
            "less_than": "must be <= effective bound",
        }.get(comparison)
        ...
        details.append(
            ...
            "----------------------------------------\n"
            f"Comparison: {comparison_label} ({comparison})\n"
            f"Configured threshold: {threshold}\n"
            f"Accepted error (%): {accepted_error}\n"
            f"Effective bound: {bound}\n"
            f"Observed value: {real_value}"
        )

    details_text = "\n\n".join(details)
    return dedent(f'''\
        Autosubmit notification: CPMIP threshold violations\n
        -------------------------\n
        Experiment id: {exp_id}\n
        Job name: {job_name}\n
