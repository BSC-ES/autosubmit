...
class Job(object):
    ...
    def __init__(self, name=None, job_id=None, status=None, priority=None, loaded_data=None):
        ...
        self.submit_time_timestamp = 0  # for wrappers, all jobs inside a wrapper are submitted at the same time
        self.start_time_timestamp = 0
        self.finish_time_timestamp = 0  # for wrappers, with inner_retrials, the submission time should be the last finish_time of the previous retrial
    ...
    def check_remote_log_exists(self) -> bool:
        """Checks if remote log file exists on remote host
    ...
    def update_submit_time_and_job_id(self, attempt: int):
        """
        Read submit data from the DB for the given attempt and update the job's
        submit_time_timestamp and job_id.

        :param attempt: The attempt (fail_count) to look up.
        :type attempt: int
        """
        submit_data = self._get_submit_data_dc_from_db(attempt)
        if submit_data:
            self.submit_time_timestamp = submit_data.submit
            self.id = submit_data.job_id
