...
The experiment IDs are generated through the command autosubmit expid.
The ID’s of Autosubmit experiments start from ``a000``
and have at least four alphanumeric characters, using digits from
...
commands this may not always be true. For example, ``autosubmit monitor a000``
works for the experiment ``a000``, but not ``autosubmit monitor A000``,
even though internally both ``a000`` and ``A000`` are stored the same
way in the SQLite database.

That is because experiment ID’s are treated as Base-36 strings,
...
Users can create “test experiments” with experiment ID’s start at ``t001``, 
“operational experiments” with experiment ID’s start at ``o001`` and 
"evaluation experiments" with IDs starting at ``e001``.
This is done via flags passed to the ``autosubmit expid`` command in the command-line.
...
``zzzz`` decoded as Base-36 integers). After the experiment ``zzzz``, the next
experiment ID generated by Autosubmit would be ``10000``, followed by ``10001``,
``10002``, and so on successively.
