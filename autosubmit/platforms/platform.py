...
class UniqueQueue(Queue):
    ...
    def put(self, job: Any, block: bool = True, timeout: float = None) -> None:
        ...
        Args:
            ...
            timeout (float): Timeout for blocking operations. Defaults to None.
        """
        if job.wrapper_type and job.wrapper_type.lower() == "vertical":  # We gather all retrials at once
...
class LookupQueue(Queue):
    ...
    def choose_action(self, requested_name: str = "", log_info: tuple[None, None] = "", block: bool = False, timeout: float = None) -> Any:
        ...
        Args:
            requested_name (String): The name of the job to be retrieved from the queue. # get & (optional) put
            ...
            timeout (float): Timeout for blocking operations. Defaults to None.
        Returns:
            Any: The job retrieved from the queue.
        """
        with self.lock:
