    ...
    def _verify_slurm_monitor(self, slurm_monitor, job_data_dc):
        try:
            if slurm_monitor.header.status not in ["COMPLETED", "FAILED"]:
                Log.warning(
                    f"Slurm status {slurm_monitor.header.status} is not COMPLETED nor FAILED for job {job_data_dc.job_name} with ID {slurm_monitor.header.name}.")
                self._log.log(
                    f"Warning on job {job_data_dc.job_name} with ssh_output {slurm_monitor.original_input}."
