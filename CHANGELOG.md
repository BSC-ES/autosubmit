...
- The dependency keyword for weak dependencies is now `WEAK` (case-insensitive); the deprecated names `FAIL_OK`/`OPTIONAL` are still accepted as aliases and normalized to `WEAK`. It marks the dependency as weak instead of accepting a failed parent.
