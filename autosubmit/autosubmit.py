...
from typing import Any, Dict, Generator, List, Optional, Set, Tuple, Union, TYPE_CHECKING
...
class Autosubmit:
        ...
        for platform_to_test in platforms_to_test:
            ...
            for job_id, jobs in queuing_jobs.items():
                ...
                if job_list.job_package_map and job_id in job_list.job_package_map:
                    wrapper_job = Autosubmit.manage_wrapper_job(as_conf, job_list, platform_to_test, job_id)
    ...
    def run_experiment(
        ...
        try:
            with Lock(lock_file, timeout=1):
                ...
                job_list.save()
                # Revise if there is any log not recovered from a previous run
        ...
        try:
            ...
            with open(rc_path, 'w') as config_file:
                ...
                try:
                    ...
                    raise AutosubmitCritical(
                        "Can not write config file: {0}", 7012, e.errno)
        ...
        try:
            ...
            with Lock(os.path.join(tmp_path, 'autosubmit.lock'), timeout=1) as fh:
                try:
                    ...
                    job_list_persistence: 'JobListPersistence' = get_job_list_persistence(expid, as_conf)
    ...
    @staticmethod
    def stop(expids_string: str, force=False, all_expids=False, force_all=False, cancel=False,
             current_status="", status="FAILED", force_yes=False) -> None:
        """The stop command allows users to stop the desired experiments.

        :param expids_string: expids to stop
        :type expids_string: str
        ...
        if all_expids:
            ...
            expid_list = expids_string.replace(',', ' ').split(' ')
