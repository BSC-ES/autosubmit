...
class Autosubmit:
    ...
    def parse_args() -> tuple[int, Optional[argparse.Namespace]]:
        ...
        try:
            ...
            subparser.add_argument(
                '--all', action="store_true", default=False, help='Get completed files to synchronize db')
            subparser.add_argument(
    ...
    def save_historical_edges(expid):
        """Save the historical edges of the experiment in the database.
    ...
    def submit_ready_jobs(as_conf: AutosubmitConfig, job_list: JobList, platforms_to_test: set[Platform], inspect=False,
        ...
        try:
            for platform in platforms_to_test:
                ...
                job_list.save_wrappers(valid_packages_to_submit, failed_packages, as_conf, preview=(inspect or only_wrappers))
                if error_message != "":
