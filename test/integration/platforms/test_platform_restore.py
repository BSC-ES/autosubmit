...
@pytest.mark.parametrize(
    "message_fragment, expected_log, expected_cm, connected",
    [
        (
...
def test_restore_platforms_connection_on_issues(
    ...
    exp.platform.connected = connected
...
@pytest.mark.parametrize(
    "message_fragment, expected_log, connected",
    [
        (
            "",
            "[MN5] Connection successful to host test",
            True,
        ),
    ],
)
def test_restore_platforms_connection_no_issues(
...
@pytest.mark.parametrize(
    "exception, expected_log, expected_cm, connected",
    [
        (
            Exception,
            "Unexpected exception sending email notification",
            "Issues while checking the connectivity of platforms.",
            False,
        ),
    ],
)
def test_restore_platforms_connection_exception_on_exception(
    ...
    exp.as_conf.get_mails_to = mocker.Mock()
    exp.as_conf.get_mails_to.return_value = exception

    mock_log_printlog = mocker.patch("autosubmit.log.log.Log.debug")

    with pytest.raises(AutosubmitCritical) as cm:
        restore_platforms([exp.platform], mail_notify=True, as_conf=None, expid=None)
