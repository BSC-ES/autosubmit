...
def test_submit_ready_jobs_checks_job_files(mocker):
    ...
    mock_check = mocker.patch(
        "autosubmit.autosubmit.check_jobs_file_exists", return_value=False
    )
    mocker.patch("autosubmit.autosubmit.JobPackager")
...
def test_describe_command_details(mocker, command, command_log):
    ...
    Path(BasicConfig.LOCAL_ROOT_DIR, "a000").mkdir(parents=True, exist_ok=True)

    _, args = Autosubmit.parse_args()
    describe_command_details(args)

    assert command_log in mock_log_info.call_args.args[0]
