...
@pytest.mark.parametrize(
    "git_operational_check_enabled,expected", [(True, AutosubmitCritical), (False, 0)]
)
@pytest.mark.git
@pytest.mark.docker
def test_git_operational_experiment_toggle_flag(
    ...
    experiment_data["CONFIG"]["GIT_OPERATIONAL_CHECK_ENABLED"] = (
        git_operational_check_enabled
    )
    ...
    args = ["-lc", "DEBUG", "-lf", "DEBUG", "run", expid]
    ...
    if type(expected) is int:
        ...
        assert (
            mocked_log.warning.mock_calls[0][1][0]
            == "Git operational check disabled by user"
        )
    else:
        r = _autosubmit(args)
        assert r == 7075
...
def test_clean_git_not_a_git_repo(
    autosubmit_exp, mocker, get_next_expid: Callable[[], str]
):
    ...
    assert (
        mocked_log.debug.call_args_list[1][0][0] == "Not a git repository... SKIPPING!"
    )
...
def test_clean_git_not_committed(
    ...
    as_conf = create_git_repository_and_server(
        tmp_path,
        autosubmit_exp,
        git_server,
        get_next_expid,
        test_clean_git_not_committed.__name__,
    )
...
def test_clean_git_not_pushed(
    ...
    as_conf = create_git_repository_and_server(
        ...
        git_server,
        get_next_expid,
        test_clean_git_not_pushed.__name__,
    )
...
def test_clean_git(
    ...
    as_conf = create_git_repository_and_server(
        tmp_path, autosubmit_exp, git_server, get_next_expid, test_clean_git.__name__
    )
