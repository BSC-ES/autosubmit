...
def test_straight_wrapper_distribution_no_wrappers(tmp_path):
    """Test that when no wrappers provided, the distribution is empty"""
    ...
    strategy = StraightWrapperAssociationStrategy(str(tmp_path))
...
def test_straight_wrapper_distribution(tmp_path):
    ...
    # the ``job_data_dc`` and the wrappers
    assert len(processed_jobs) == 1 + len(job_data_dcs_in_wrapper)
    assert job_data_dc.rowstatus == RowStatus.PROCESSED

    # as we have 2 jobs, straight strategy gives a weight of 1.0 each, which results in splitting equals the weights,
    # so in a total of 2 jobs, each will have half of the total 1.0, so 0.5 each.
    weight = 0.5
    extern_times_weight = weight * slurm_monitor.extern.energy
    assert job_data_dcs_in_wrapper[0].energy == slurm_monitor.steps[0].energy + extern_times_weight
    assert job_data_dcs_in_wrapper[1].energy == slurm_monitor.steps[1].energy + extern_times_weight

    # It simply gets the last job that matches its name
    assert job_data_dc.energy == job_data_dcs_in_wrapper[1].energy
