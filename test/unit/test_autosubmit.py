...
# noinspection PyProtectedMember
from autosubmit.workflow.manage import _finish_current_experiment_run, _signal_handler
...
def test_copy_as_config(autosubmit_config: AutosubmitConfigFactory, monkeypatch):
    """Test ``copy_as_config``."""
    autosubmit_config("a000", {})
    monkeypatch.setattr(BasicConfig, "LOCAL_ROOT_DIR", f"{BasicConfig.LOCAL_ROOT_DIR}")
...
def test_copy_as_config_invalid(
    autosubmit_config: AutosubmitConfigFactory, mocker, monkeypatch
):
...
def test_database_backup_postgres(monkeypatch, mocker):
    ...
    mocked_log = mocker.patch("autosubmit.database.db_common.Log")
...
def test_signal_handler_sets_exit_flag(monkeypatch):
...
def test_check_non_wrapped_jobs_calls_platform_and_updates_status(
    ...
    check_non_wrapped_jobs([fake_platform], fake_job_list, as_conf, "a000")  # type: ignore

    fake_platform.check_all_jobs.assert_called_once()
    fake_job_list.save_jobs.assert_called_once()  # type: ignore
...
def test_submit_ready_jobs_inspect_skips_check(mocker):
    ...
    check_jobs_file_exists.assert_not_called()  # type: ignore
...
def test_check_non_wrapped_jobs_no_status_change(mocker, fake_job_list, fake_platform):
    ...
    fake_job_list.save_jobs.assert_not_called()  # type: ignore
...
def test_wrapper_notify_calls_job_notify_for_each_inner(mocker):
    ...
    assert 2 == mock_job_notify.call_count
...
def test_sigint_handler_sets_exit_flag(monkeypatch):
