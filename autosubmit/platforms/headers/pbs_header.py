...
class PBSHeader(object):
    ...
    def get_nodes_directive(job, parameters, het=-1):
        ...
        """
        if parameters['NODES'] != '':
            return f":nodes={parameters['NODES']}"
        return ""
    ...
    def get_threads_per_task(job, parameters, het=-1):
        ...
        if parameters['NUMTHREADS'] != '':
            return f":mpiprocs={parameters['NUMTHREADS']}"
        return ""
    ...
    def get_tasks_per_node(job, parameters, het=-1):
        ...
        if int(parameters['TASKS']) > 1:
            return f"PBS -l select={parameters['CURRENT_NODES']}"
        return ""
    ...
    SERIAL = textwrap.dedent("""\
            ...
            #!/bin/sh --login
            #
            #%TASKS_PER_NODE_DIRECTIVE%%NODES_DIRECTIVE%%THREADS_PER_TASK_DIRECTIVE%
            #%QUEUE_DIRECTIVE%
            #%ACCOUNT_DIRECTIVE%
