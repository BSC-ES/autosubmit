...
class TestExperimentHistory:
    ...
    def setup_method(self):
        ...
        self.job_list = [
            ...
            job(
                "a29z_20000101_fc3_1_POST", "2000-01-01 00:00:00", "POST", "RUNNING", ""
            ),
            job(
                "a29z_20000101_fc2_1_CLEAN",
                "2000-01-01 00:00:00",
                "CLEAN",
                "COMPLETED",
                "",
            ...
            job(
                "a29z_20000101_fc0_3_SIM", "2000-01-01 00:00:00", "SIM", "COMPLETED", ""
            ),
            job(
                "a29z_20000101_fc1_2_POST", "2000-01-01 00:00:00", "POST", "QUEUING", ""
            ),
        ...
        self.job_list_large = [
            ...
            job(
                "a29z_20000101_fc1_4_POST", "2000-01-01 00:00:00", "POST", "FAILED", ""
            ),
            job(
                "a29z_20000101_fc2_5_CLEAN",
                "2000-01-01 00:00:00",
                "CLEAN",
                "SUBMITTED",
                "",
            ...
            job(
                "a29z_20000101_fc0_1_POST", "2000-01-01 00:00:00", "POST", "RUNNING", ""
            ),
        ]
    ...
    def test_should_we_create_new_run(self):
        ...
        current_experiment_run_dc = (
            exp_history.manager.get_experiment_run_dc_with_max_id()
        )
        current_experiment_run_dc.total = TOTAL_COUNT
        should_we = exp_history.should_we_create_a_new_run(
            self.job_list,
            CHANGES_COUNT,
            current_experiment_run_dc,
    ...
    def test_process_job_list_changes(self):
        exp_history = ExperimentHistory("tt00")
        exp_history.initialize_database()
        exp_run = exp_history.process_job_list_changes_to_experiment_totals(
            self.job_list
        )
        assert exp_run.total == len(self.job_list)
        assert exp_run.completed == 4
    ...
    """
        ...
        weights = StraightWrapperAssociationStrategy().get_calculated_weights_of_jobs_in_wrapper(
            job_data_dcs
        )
        info_handler = PlatformInformationHandler(StraightWrapperAssociationStrategy())
        job_data_dcs_with_data = info_handler.execute_distribution(
            job_data_dcs[0], job_data_dcs, slurm_monitor
        )
        assert job_data_dcs_with_data[0].energy == round(
            slurm_monitor.steps[0].energy
            + weights[job_data_dcs_with_data[0].job_name] * slurm_monitor.extern.energy,
            2,
        )
        assert job_data_dcs_with_data[0].MaxRSS == slurm_monitor.steps[0].MaxRSS
        assert job_data_dcs_with_data[2].energy == round(
            slurm_monitor.steps[2].energy
            + weights[job_data_dcs_with_data[2].job_name] * slurm_monitor.extern.energy,
            2,
        )
    ...
    """
        ...
        weights = GeneralizedWrapperDistributionStrategy().get_calculated_weights_of_jobs_in_wrapper(
            job_data_dcs
        )
        ...
        sum_energy = sum(
            job.energy for job in job_data_dcs_with_data[:5]
        )  # Last 1 is original job_data_dc
    ...
    def test_process_status_changes(self):
        ...
        exp_run = exp_history.process_status_changes(
        ...
        new_job_list = [
            job(
                "a29z_20000101_fc2_1_POST", "2000-01-01 00:00:00", "POST", "FAILED", ""
            ),
            job(
                "a29z_20000101_fc1_1_CLEAN",
                "2000-01-01 00:00:00",
    ...
    def test_write_submit_time(self):
        ...
        JOB_ID = 101
        inserted_job_data_dc = exp_history.write_submit_time(
            JOB_NAME,
            time.time(),
            "SUBMITTED",
            NCPUS,
    ...
    def test_write_start_time(self):
        ...
        inserted_job_data_dc = exp_history.write_start_time(
            JOB_NAME,
            time.time(),
            "RUNNING",
            NCPUS,
...
def test_get_finish_data_dc(tmp_path, monkeypatch):
    ...
    assert inserted_job_data_dc is not None, (
        "write_finish_time returned None; check for internal errors."
    )

    finish_data_dc = exp_history.get_finish_data_dc(JOB_NAME, fail_count=FAIL_COUNT)
    assert finish_data_dc is not None, (
        "get_finish_data_dc returned None; record not found."
    )
...
def test_update_submit_time(tmp_path, monkeypatch):
    ...
    assert finish_data_dc.submit == new_submit_time, (
        f"Expected submit time {new_submit_time}, got {finish_data_dc.submit}"
    )
