...
class Job(object):
    ...
    def __init__(self, name=None, job_id=None, status=None, priority=None, loaded_data=None) -> None:
        ...
        self.finish_time_timestamp = 0.0 # for wrappers, with inner_retrials, the submission time should be the last finish_time of the previous retrial
    ...
    def update_status(self, as_conf: AutosubmitConfig, failed_file: bool = False) -> Status:
        ...
        if new_status == Status.COMPLETED:
            ...
            self._platform.get_completed_files(self.name, wrapper_failed=self.packed) # type: ignore
        ...
        if self.status == Status.RUNNING:
            ...
            Log.printlog("Job {0} is UNKNOWN. Checking completed files to confirm the failure...".format(
                self.name), 3000)
            self._platform.get_completed_files( # type: ignore
        ...
        if self.status in [Status.COMPLETED, Status.FAILED, Status.UNKNOWN]:
            if str(as_conf.platforms_data.get(self.platform.name, {}).get('DISABLE_RECOVERY_THREADS', "false")).lower() == "true":
                self.retrieve_logfiles(self.platform is not None)
    ...
    def _get_paramiko_template(self, snippet: 'TemplateSnippet', template, parameters) -> str:
        ...
        return ''.join([
            snippet.as_header(current_platform.get_header(self, parameters), self.executable), # type: ignore
