...
class PBSPlatform(ParamikoPlatform):
    ...
    def update_cmds(self) -> None:
        ...
        self._submit_cmd = (f'qsub -l select={self.config.get("PLATFORMS", {}).get(self.name.upper(), {}).get("NODES", "2")}'
                            f':mpiprocs={self.config.get("PLATFORMS", {}).get(self.name.upper(), {}).get("TASKS", "4")}'
                            f' {self.remote_log_dir}/')
