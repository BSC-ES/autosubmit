...
class ExperimentHistoryDbManager(DatabaseManager):
    ...
    def __init__(self, expid, jobdata_dir_path=DEFAULT_JOBDATA_DIR):
        ...
        self.historicaldb_file_path = os.path.join(self.JOBDATA_DIR,
                                                   "job_data_{0}.db".format(self.expid))
    ...
    def _set_table_queries(self):
        ...
        self.create_table_query = textwrap.dedent(
            ...
            ''')
        self.create_index_query = textwrap.dedent('''CREATE INDEX IF NOT EXISTS
        ID_JOB_NAME ON job_data(job_name);''')
    ...
    def update_historical_database(self):
        ...
        self.execute_many_statements_on_dbfile(self.historicaldb_file_path,
                                               self.version_schema_changes)
        self.execute_statement_on_dbfile(self.historicaldb_file_path, self.create_index_query)
        self.execute_statement_on_dbfile(self.historicaldb_file_path,
                                         self.create_table_header_query)
    ...
    def _get_experiment_run_with_max_id(self):
        ...
        statement = self.get_built_select_statement("experiment_run",
                                                    "run_id > 0 ORDER BY run_id DESC LIMIT 0, 1")
    ...
    def _get_job_data_last_by_wrapper_code(self, wrapper_code):
        ...
        statement = self.get_built_select_statement("job_data",
                                                    "rowtype = ? and last=1 ORDER BY id")
        arguments = (wrapper_code,)
        job_data_rows = self.get_from_statement_with_arguments(self.historicaldb_file_path,
                                                               statement, arguments)
        return [Models.JobDataRow(*row) for row in job_data_rows]
    ...
    def _insert_job_data(self, job_data):
        ...
        statement = ''' INSERT INTO job_data(counter, job_name, created, modified,
                ...
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) '''
        arguments = (job_data.counter, job_data.job_name, HUtils.get_current_datetime(),
                     HUtils.get_current_datetime(), job_data.submit, job_data.start,
                     job_data.finish, job_data.status, job_data.rowtype, job_data.ncpus,
                     job_data.wallclock, job_data.qos, job_data.energy, job_data.date,
                     job_data.section, job_data.member, job_data.chunk, job_data.last,
                     job_data.platform, job_data.job_id, job_data.extra_data, job_data.nnodes,
    ...
    def _insert_experiment_run(self, experiment_run):
        ...
        arguments = (HUtils.get_current_datetime(), HUtils.get_current_datetime(),
                     experiment_run.start, experiment_run.finish, experiment_run.chunk_unit,
                     experiment_run.chunk_size, experiment_run.completed, experiment_run.total,
    ...
    def _update_job_data_by_id(self, job_data_dc: Any) -> None:
        ...
        Update last, submit, start, finish, modified, job_id,
        status, energy, extra_data, nnodes, ncpus, rowstatus, out, err by id.

        Args:
            job_data_dc (JobData): The JobData data class
            instance containing job data to be updated.
        Returns:
            None
        """
    ...
    def _update_experiment_run(self, experiment_run_dc):
        ...
        Updates by run_id (finish, chunk_unit, chunk_size, completed,
        total, failed, queuing, running, submitted, suspended)
        """
        statement = ''' UPDATE experiment_run SET finish=?, chunk_unit=?, chunk_size=?,
            completed=?, total=?, failed=?, queuing=?, running=?, submitted=?, 
            suspended=?, modified=? WHERE run_id=? '''
        arguments = (experiment_run_dc.finish, experiment_run_dc.chunk_unit,
                     experiment_run_dc.chunk_size, experiment_run_dc.completed,
                     experiment_run_dc.total, experiment_run_dc.failed, experiment_run_dc.queuing,
                     experiment_run_dc.running, experiment_run_dc.submitted,
    ...
    def _get_pragma_version(self):
        ...
        if len(pragma_result) <= 0:
            raise Exception(
                ...
                "This might be a signal of a deeper problem. Review previous errors.")
