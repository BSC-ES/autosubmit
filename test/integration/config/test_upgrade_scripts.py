...
def test_upgrade_scripts(autosubmit_exp, tmp_path: 'LocalPath', as3_ini_files: list[Path], mocker):
    ...
    assert mocked_log.printlog.called
    assert any('Duplicate variable found: USER' in str(c) for c in mocked_log.printlog.call_args_list)
    ...
    assert '%ROOTDIR%' in new_local_script_content, "The %rootdir% variable was not upgraded/upper-cased!"
